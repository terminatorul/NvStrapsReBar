...
#### Warning: crash on resume from suspend
Some users report BSOD or crash when resuming from suspend, which can be even worse for laptop users that are trying to stay mobile. Developing a boot script for the DXE driver might address the issue, but the feautre is still under development and there is currently not fix for this issue.
